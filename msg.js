...
const sms = (conn, m) => {
    if (m.key) {
        ...
        m.chatAlt = m.key.remoteJidAlt || null
        m.fromMe = m.key.fromMe
        m.isGroup = m.chat.endsWith('@g.us')
        
        const participant = m.key.participant || null
        const participantAlt = m.key.participantAlt || null
        const senderPn = m.key.senderPn || null
        const participantPn = m.key.participantPn || null
        
        let rawSender, rawSenderAlt, priorityPn
        if (m.fromMe) {
            rawSender = conn.user.id
            rawSenderAlt = null
            priorityPn = null
        } else if (m.isGroup) {
            rawSender = participant || m.key.remoteJid
            rawSenderAlt = participantAlt
            priorityPn = participantPn
        } else {
            rawSender = m.key.remoteJid
            rawSenderAlt = m.key.remoteJidAlt || null
            priorityPn = senderPn
        }
        
        m.senderLid = isLidFormat(rawSender) ? rawSender : (isLidFormat(rawSenderAlt) ? rawSenderAlt : null)
        m.senderPn = priorityPn || (isPnFormat(rawSender) ? rawSender : (isPnFormat(rawSenderAlt) ? rawSenderAlt : null))
        m.senderNumber = extractNumber(m.senderPn || rawSender)
        m.senderRaw = rawSender
        m.senderAlt = rawSenderAlt
        m.sender = m.senderPn ? m.senderPn : (m.senderNumber ? m.senderNumber + '@s.whatsapp.net' : rawSender)
    }
    if (m.message) {
        ...
        if (m.msg) {
            ...
            if (m.quoted) {
                ...
                m.quoted.senderAlt = m.msg.contextInfo.participantAlt || null
                m.quoted.senderLid = isLidFormat(m.quoted.sender) ? m.quoted.sender : (isLidFormat(m.quoted.senderAlt) ? m.quoted.senderAlt : null)
                m.quoted.senderPn = isPnFormat(m.quoted.sender) ? m.quoted.sender : (isPnFormat(m.quoted.senderAlt) ? m.quoted.senderAlt : null)
                m.quoted.senderNumber = extractNumber(m.quoted.senderPn || m.quoted.sender)
                m.quoted.fromMe = m.quoted.senderNumber === extractNumber(conn.user.id)
